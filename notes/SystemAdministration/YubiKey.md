...
tags:
  ...
  - life-in-the-cloud
post: true
post-title: yubikey-neo-2-factor-authentication-u2f
...
# Yubikey: 2-factor authentication (U2F, OATH and OTP) and SSH
...
On Omarchy in 2026, no udev rules are needed. The following seems obsolete:
...
For sites that do not support U2F or codes-based 2-factor authentication, the only line of defense is the password, so
...
# generate the key
$ ssh-keygen -t ed25519-sk -O resident -O application=ssh:dub@podval.org -O user=dub@podval.org -C "ssh key for dub@podval.org on Yubikey"
